...
//Info
//FALTA: Hacer que sume el MVP, HACER QUE SUME EL SCORE
public Plugin:myinfo =
{
	name = "ScoreSoccer v1.32",
	author = "arbol",
...
public Action:awardPeople(Handle:timer, any:userid)
{
    ...
    if(client > 0)
    {
        ...
        if ( (IsPlayerAlive(client)) && (GetClientTeam(client) > 1)){
            ...
            new killss = GetClientFrags(client);
            ...
            SetClientFrags(client, killss+1);
            CS_SetClientAssists(client,classist+1);
            CS_SetMVPCount(client, emevepe+1);
            CS_SetClientContributionScore(client, contrscore+1);
            PrintToChat(client, "scoreSoccer: Processing done. %d %d %d %d",GetClientFrags(client),CS_GetClientAssists(client),CS_GetMVPCount(client),CS_GetClientContributionScore(client)  );
        }
    }
}
