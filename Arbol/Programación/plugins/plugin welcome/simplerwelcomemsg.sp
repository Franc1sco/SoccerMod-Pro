...
new String:name[MAX_NAME_LENGTH], String:language[MAX_NAME_LENGTH], String:language2[MAX_NAME_LENGTH], String:secondMessage[128], String:thirdMessage[128]
new String:fourthMessage[256]
...
public Plugin:myinfo =
{
	...
	author = "simoneaolson / modif: arbol",
...
public Action:TimerWelcomeHint(Handle:timer, any:clientSerial)
{
	...
	{//observación: el secondMessage está bugeado, no admite más de 32 car. Ídem third message. El 4 puede ser tan largo como se requiera.
    //es un tema de que el último puede ser tan largo como se requiera pero los otros no. No sé bien por qué sucede.
		GetClientName(client, name, 32);
		Format(language, 32, "%t", "Welcome");
		Format(language2, 32, "%t", "2Welcome2");
		Format(secondMessage, 128 , "%t", "SecondMessage");
		Format(thirdMessage, 128, "%t", "ThirdMessage");
		Format(fourthMessage, 256, "%t", "FourthMessage");
		PrintHintText(client, "%s %s %s", language, name,  language2);
		PrintToChat(client, "%s", secondMessage);
		PrintToChat(client, "%s", thirdMessage);
		PrintToChat(client, "%s", fourthMessage);
	}
